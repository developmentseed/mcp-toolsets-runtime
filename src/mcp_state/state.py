...
class StateEntry(TypedDict):
    ...
    #: turn-scoped read can fetch. Absent means one, so an entry carrying no
    #: count reads as the only turn to have written its key. Read it through
    #: :func:`turns_written`.
...
def turns_written(entry: StateEntry | None) -> int:
    ...
    ``1`` for a key written in one turn, and for an entry that carries no count
    at all — the honest default, since a listing that flagged every entry it
    could not vouch for would train a model to ignore the flag.
    """
