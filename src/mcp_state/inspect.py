...
def _at_turn(as_of: int | None) -> dict[str, int]:
    """The field naming which turn an answer without a value is about.

    Every other answer here carries its turn in the value's trailing note, but
    a listing and a miss have no value to append to — and they are the two
    answers a model is most likely to read as being about the present. Left
    unlabelled, "this key is not stored" and "these are the keys" both come
    back looking exactly as they would from a read of now, which is the
    well-formed wrong answer this whole path exists to remove.
    """
    return {} if as_of is None else {"at_turn": as_of}


def _all_keys(as_of: int | None) -> str:
    """What a grep across every key searched, named so the turn is not lost."""
    return "all keys" if as_of is None else f"all keys at the end of turn {as_of}"
...
def read_state_key(
    ...
    *,
    ...
    "the above is the latest" is false of a value read out of the past. The
    answers with no value to append a note to — a key listing, and a key that
    misses — carry the turn as a field instead, so neither reads as an answer
    about the present.
    ...
    if key in (ALL_KEYS, ""):
        ...
        if pattern:
            ...
            return _grep_response(pattern, _all_keys(as_of), pairs, populated)
        return _dumps(
            {**_at_turn(as_of), "available_keys": _summaries(populated, stored)}
        )
    ...
    if key not in populated:
        ...
        # Nothing there *then* is not nothing there *now*: a key first written
        # later reads as unknown here, and the advice a present-tense miss
        # carries — run the tool that produces it — would have the model fill a
        # key that is already full.
        if as_of is not None:
            response["reason"] = (
                f"No value was stored under this key at the end of turn {as_of}. "
                "A later turn may have written it — read the key without turn "
                "for what it holds now."
            )
...
async def read_state_key_at_turn(
    ...
    *,
    ...
    found = await history.snapshot(thread_id, turn)
    if found.never_had(turn):
        return _dumps(
            {
                "no_such_turn": turn,
                "turns_so_far": found.total,
                "reason": (
                    "Turns are the user's questions, counted from 1. This "
                    f"conversation has had {found.total}."
                ),
            }
        )
        ...
        return _dumps(
            {
                ...
                "retained_turns": sorted(found.retained),
                "reason": (
    ...
    return read_state_key(
        key, {TOOL_STATE_KEY: dict(found.state)}, pattern=pattern, path=path, as_of=turn
    )
...
def make_inspect_state(
    ...
    turn*, and is optional. Without one the tool reads the present and nothing
    else, and ``turn=`` answers that this deployment keeps no turn history — so
    a host with no checkpointer has a working tool and needs to supply nothing.
    See :mod:`mcp_state.history`.
    """
