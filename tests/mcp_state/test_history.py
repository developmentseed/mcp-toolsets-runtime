...
class FakeHistory:
    ...
    Built from every turn it holds and narrowing per call, which is the shape
    a real store has: the host knows them all, and the protocol is what stops
    them all travelling to answer about one.
    """

    def __init__(self, turns: Mapping[int, dict], total: int) -> None:
        self._turns = turns
        self._total = total
    ...
    async def snapshot(self, thread_id: str, turn: int) -> Snapshot:
        ...
        return Snapshot(
            ...
            retained=frozenset(self._turns),
            total=self._total,
        )
...
async def test_a_retained_turn_reads_back_as_that_turn_left_it() -> None:
    history = FakeHistory(
        {1: {KEY: _entry(COPENHAGEN)}, 2: {KEY: _entry(AARHUS)}}, total=2
    )
...
async def test_narrowing_works_the_same_at_a_turn_as_it_does_now() -> None:
    ...
    history = FakeHistory({1: state}, total=1)
...
async def test_a_pruned_turn_is_not_reported_as_one_that_never_existed() -> None:
    ...
    """
    history = FakeHistory({3: {KEY: _entry(AARHUS)}}, total=3)
...
async def test_turn_zero_is_a_miscount_not_a_read() -> None:
    ...
    history = FakeHistory({1: {KEY: _entry(COPENHAGEN)}}, total=1)
...
async def test_an_unknown_key_at_a_turn_lists_what_that_turn_held() -> None:
    ...
    history = FakeHistory(
        {1: {KEY: _entry(COPENHAGEN)}, 2: {"gazet/other/x": _entry(1)}}, 2
    )
    ...
    # And says so. A miss carries no value to hang the "as it stood at" note
    # on, so without this the answer is shaped exactly like a read of the
    # present that found nothing — and the model acts on the wrong one.
    assert answer["at_turn"] == 2
    assert "at the end of turn 2" in answer["reason"]
    assert "without turn" in answer["reason"]


async def test_a_key_listing_at_a_turn_is_not_mistaken_for_the_present() -> None:
    """The keys a turn held are not the keys held now, and the answer says which."""
    history = FakeHistory(
        {1: {KEY: _entry(COPENHAGEN)}, 2: {"gazet/other/x": _entry(1)}}, 2
    )

    listing = json.loads(await read_state_key_at_turn("*", 1, THREAD, history))
    assert listing["at_turn"] == 1
    assert list(listing["available_keys"]) == [KEY]

    # A read of now carries no such field: there is no turn to name.
    assert "at_turn" not in json.loads(read_state_key("*", {TOOL_STATE_KEY: {}}))


async def test_a_grep_across_every_key_at_a_turn_names_the_turn() -> None:
    """Grep answers in prose, so the turn goes in the header rather than a field."""
    history = FakeHistory({1: {KEY: _entry({"name": "København"})}}, total=1)

    found = await read_state_key_at_turn("*", 1, THREAD, history, pattern="Kø")
    assert "all keys at the end of turn 1" in found
    assert "all keys at the end of turn" not in read_state_key(
        "*", {TOOL_STATE_KEY: {KEY: _entry({"name": "København"})}}, pattern="Kø"
    )
...
async def test_a_turn_scoped_read_names_its_turn_and_claims_no_latest() -> None:
    ...
    history = FakeHistory({1: {KEY: _entry(COPENHAGEN)}, 2: {KEY: at_two}}, total=2)
