...
they happen, the session-state panel, `ui://` views in their frames, and the
questions the model asks with `interrupt_gate` (see
[docs/CONSUMING.md §5g](./docs/CONSUMING.md#5g-questions-from-the-model-interrupt_gate)).
No Node runs in the image and no front end is copied into the deployment.
