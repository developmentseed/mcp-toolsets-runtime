...
`@modelcontextprotocol/ext-apps`, which `mcp_agent/elements/McpView.jsx` uses —
the same four messages, hand-written, so the client carries no second protocol
library for the one frame it opens.
...
host uses, so the wire and the bundled UI cannot drift. `js/agent-ui/src/chat.tsx` prints it;
