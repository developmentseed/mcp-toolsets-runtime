...
class Snapshot:
    state: Mapping[str, StateEntry] | None  # that turn's, or None
    retained: frozenset[int]  # the turns still held, numbers only
    total: int  # how many the thread has had
```

Keys and entries, nothing else, so `mcp_state` still knows nothing about
threads, checkpoints or retention. It asks for one turn rather than all of
them because session state holds precisely the values too big for a
transcript, and handing back every turn's to answer about one multiplies the
largest thing here by the length of the conversation. `total` alongside the
state is what lets it tell a turn the thread never had from one that has been
pruned:
